...
def find_objects(action, invariant):
    objects = []
    for parameter in action.parameters:
        objects.append(parameter.type_name)
    for literal in action.preconditions.parts + [eff.literal for eff in action.effects]:
        objects.append(literal.predicate)
        for arg in literal.args:
            if not arg.startswith("?"):
                objects.append(arg)
    return objects

def find_substitutions(action, invariant):
    objects = find_objects(action, invariant)
    variables = invariants.find_variables(invariant)
    variables_temp = []
    for var in variables:
        temp = list(itertools.product([var], objects))
        variables_temp.append(temp)
    substitutions = list(itertools.product(*variables_temp))
    result = []
