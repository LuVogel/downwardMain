...
def weaken2(inv_cand: InvariantCandidate):
    ...
    if len(inv_cand.parts) == 1:
        ...
        for pred, type_names in predicates_in_task.items():
            ...
            for args in params:
                ...
                arg_temp = []
                for arg in args:
                    arg_temp.append(TypedObject(name=arg, type_name=type_names[type_counter]))
                    type_counter += 1
                pos = Atom(pred, arg_temp)
                neg = NegatedAtom(pred, arg_temp)
                inv_cand_set.add(InvariantCandidate(parts=inv_cand.parts+(pos,)))
                inv_cand_set.add(InvariantCandidate(parts=inv_cand.parts+(neg,)))
    ...
    # existing_inequalities = set() # set(inv_cand.inequalities)
...
def weaken(inv_cand: InvariantCandidate, action: PropositionalAction):
    ...
    for cond, eff in action.add_effects:
        if eff not in curr_inv_list:
            ...
            new_inv_cand = InvariantCandidate(parts=curr_inv_list)
...
def create_grounded_invariant_for_c_sigma(list_of_vars, predicate, negated):
    ...
    if count_of_lists == 1:
        for var in list_of_vars[0]:
            if negated:
                inv_list.append(InvariantCandidate(parts=[NegatedAtom(predicate=predicate, args=[var])]))
            else:
                inv_list.append(InvariantCandidate(parts=[Atom(predicate=predicate, args=[var])]))
        return inv_list

    for combination in itertools.product(*list_of_vars):
        inv_list.append(list(combination))
...
def parse_objects_with_current_pred(task_pred: Predicate, map):
    ...
    if count_pred_in_init == p_should_have_length:
        return InvariantCandidate(parts=[Atom(predicate=task_pred.name, args=task_pred.arguments)])
    elif count_pred_in_init == 0:
        return InvariantCandidate(parts=[NegatedAtom(predicate=task_pred.name, args=task_pred.arguments)])
    else:
        return None
    ...
    while True:
        ...
        for action in list_of_possible_actions:
            ...
            while len(queue_cq) != 0:
                ...
                print("removed inv_cand from queue and is now current inv_cand in loop: ")
                inv_cand.dump()
